...
const ManageContests = () => {
  ...
  const { data: contestsData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosInstance.get("/api/admin/contests?limit=100");
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({ id, approved, status }) => {
      await axiosInstance.put(`/api/admin/contests/${id}/status`, {
        approved,
        status,
      });
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id) => {
      await axiosInstance.delete(`/api/admin/contests/${id}`);
    },
    onSuccess: () => {
  ...
  const handleContestAction = (id, action) => {
    if (action === "delete") {
      Swal.fire({
        ...
        text: "This will permanently remove the contest.",
        icon: "warning",
    ...
    const approved = action === "Confirmed";
    Swal.fire({ title: `${action} contest?`, showCancelButton: true }).then(
      (r) => {
        if (r.isConfirmed)
          updateStatusMutation.mutate({ id, approved, status: action });
      }
    );
  };
